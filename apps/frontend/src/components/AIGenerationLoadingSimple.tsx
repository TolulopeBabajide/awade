...
interface AIGenerationLoadingSimpleProps {
  isVisible: boolean;
  onComplete?: () => void;
...
const AIGenerationLoadingSimple: React.FC<AIGenerationLoadingSimpleProps> = ({
  ...
  useEffect(() => {
    ...
    let stepIndex = 0;
    const stepDurations = [1000, 800, 1200, 3000, 1000, 500]; // Duration for each step in ms
    ...
    const progressInterval = setInterval(() => {
      if (stepIndex < steps.length) {
        ...
        const stepProgress = ((stepIndex + 0.5) / steps.length) * 100;
        ...
        setTimeout(() => {
          ...
          // Update progress to 100% when all steps are done
          if (stepIndex >= steps.length) {
            setProgress(100);
            setTimeout(() => {
              onComplete?.();
...
export default AIGenerationLoadingSimple;
