...
> Last updated: 2026-05-08 (Lead Dev Agent — AWD-M-81 resolved: replaced bare `} catch {` in `handleConsentConfirmed` with `} catch (err) {` and `setConsentError(err instanceof Error ? err.message : 'Something went wrong. Please try again.')`. Network errors now surface their message in the consent modal's `<p role="alert">` instead of being collapsed into a generic string. 2 new tests in `ParentDashboardPage.test.tsx` (Error-instance message, non-Error fallback). Commit `c34ba38`, merge `987d89a` (via `git commit-tree` + ref-file overwrite — virtiofs FUSE keeps `.git/index.lock` undeletable). AWD-C-13 did NOT trigger this run — index was clean (the post-commit cached-diff was the stale on-disk index from before my plumbing commits, refreshed via `git read-tree HEAD` into a temp file then `cp` into `.git/index`). Frontend `tsc --noEmit` 0 errors · `eslint --max-warnings 0` 0 errors. Vitest could not run in sandbox (ENOSPC tmp-dir, M-85 still); Render CI will validate. JSON validity (`openapi.json`, `.cursor/mcp.json`) ✅. Tolu: run `git push origin develop` to trigger CI. H-65 and M-77 still blocked by Tolu's venv fix.)
...
> Last updated: 2026-05-05 (code-review-agent — AWD-M-107 + AWD-M-98 verified resolved (commits f33aa84→0ebfb6c). Filed AWD-M-108: auth_service.py 655 lines exceeds 400-line threshold. Filed AWD-M-109: token_payload dict duplicated 4×. Verdict: ✅ Clean. H-65 and M-77 still blocked by Tolu's venv fix.)
...
| M-108 | Code Quality / Architecture | **`auth_service.py` is 655 lines — exceeds the 400-line split threshold.** The file hosts four structurally independent concerns: Google OAuth flow, email registration, email login, and token lifecycle (create/refresh/blacklist). A field added to the token payload, a change to error-handling strategy, or a new auth flow requires navigating a 655-line file and risks unintended side-effects. **Fix**: Extract token-lifecycle methods (`create_access_token`, `create_refresh_token`, `blacklist_refresh_token`, `is_refresh_token_blacklisted`, `refresh_access_token`) into a `TokenService` class in `apps/backend/services/token_service.py`. `AuthService` retains user-identity operations. No behaviour change required. Filed: 2026-05-05 code-review-agent. | `apps/backend/services/auth_service.py` | M | Stage: define |
| M-116 | Testing / Code Quality | **`test_children_router.py` is 759 lines — exceeds the 400-line split threshold.** The file covers six distinct test groups (auth gating, role enforcement, ownership, CRUD, guide idempotency/502, PDF export, rate-limit structure). As consent-flow and quota tests are added (AWD-M-89), the file will continue to expand. **Fix**: Split into `tests/test_children_auth.py`, `tests/test_children_crud.py`, `tests/test_children_guides.py`, `tests/test_children_export.py`, `tests/test_children_rate_limits.py`; move `_make_user`, `_make_child_profile`, `_make_guide` factories into `conftest.py`. No logic change required. Filed: 2026-05-06 code-review-agent. | `apps/backend/tests/test_children_router.py` | M | Stage: define |
| M-110 | Code Quality / Architecture | **`test_services.py` is 626 lines — exceeds the 400-line split threshold.** The file exercises five distinct service classes (`AuthService`, `UserService`, `LessonPlanService`, `ContextService`, `OptimizedDatabaseService` / data-structures) in a single module. As `AuthService` test coverage grows (it is now the dominant section), the file will continue to expand and slow `pytest` collection. **Fix**: Split into `tests/test_auth_service.py`, `tests/test_user_service.py`, `tests/test_lesson_plan_service.py`, `tests/test_context_service.py`, `tests/test_data_structures.py` — move `conftest.py` fixtures up so all modules share them. No logic change required. Filed: 2026-05-05 code-review-agent. | `apps/backend/tests/test_services.py` | M | Stage: define |
| M-117 | Code Quality / Architecture | **`lesson_plan_service.py` is 598 lines — exceeds the 400-line split threshold.** The file hosts three structurally independent concerns: (a) `LessonPlan` CRUD + AI generation (`generate_lesson_plan`, `get_lesson_plans`, `get_lesson_plan`, `update_lesson_plan`, `delete_lesson_plan`, `create_lesson_plan_response`, `fetch_curriculum_data`), (b) `LessonResource` CRUD + access control (`generate_lesson_resource`, `get_all_lesson_resources`, `get_lesson_plan_resources`, `get_lesson_resource_orm`, `get_lesson_resource`), and (c) ad-hoc helpers. Same shape as already-filed AWD-M-108 (`auth_service.py` 655 lines). The AWD-M-70 helper extraction was the right move but lengthened this file rather than splitting it. **Fix**: Extract resource methods (lines 325–598) into `apps/backend/services/lesson_resource_service.py::LessonResourceService` taking the same `(db, redis_pool)` constructor; `LessonPlanService` retains plan CRUD + AI generation. Update `routers/lesson_plans.py` resource endpoints to use the new service. No behaviour change. Filed: 2026-05-06 code-review-agent. | `apps/backend/services/lesson_plan_service.py` | M | Stage: define |
...
**[AWD-M-112]** — DepSec: Pillow@10.4.0 → 12.2.0 — CVE-2026-40192, CVE-2026-25990, CVE-2026-42311, CVE-2026-42310, CVE-2026-42308 (5 CVEs incl. network decompression bomb) | Stage: define  
- CVE-2026-40192 is network-accessible (FITS GZIP decompression bomb, AV:N, VA:H). Others are local. All fixed in 12.2.0. Major version jump (10→12) — needs API compat review in `apps/backend/services/pdf_service.py` and any image-handling code before patching.
