...
> Last updated: 2026-04-24 (Dev Agent — AWD-H-36 restore batch subject FK query committed b25e3a0 / merge 67d23ce)
> Last groomed: 2026-04-21 (weekend-ops / Ops Agent) — no changes to scope; backlog is freshly populated and aligned with the parent pivot.
...
| M-03 | DX | Pre-commit hooks for lint + type check (husky + lint-staged) | `.husky/`, `apps/frontend/package.json` | S |
| M-04 | Testing | Backend coverage below 70% threshold in some modules — shore up children_service + lesson_plan_service | `apps/backend/tests/` | M |
| M-05 | Parents | Share-to-WhatsApp button on parent guides (high-engagement channel in target markets) | `apps/frontend/src/pages/GuideViewPage.tsx` | S |
...
| M-21 | Parents | Guide export — add PDF/DOCX export button to GuideViewPage so parents can print guides for offline use (rebranding doc §5.1 confirms export service carries over) | `apps/frontend/src/pages/GuideViewPage.tsx`, `apps/backend/routers/children.py` (new export endpoint) | M |
...
| L-10 | Docs / Config | `project-config.md` §5 `ERROR_MONITORING` still reads "not yet connected (Sentry recommended — flagged as H-01)". AWD-H-01 shipped in commit 364762f — update the line to reflect Sentry is now wired for both backend (`sentry-sdk[fastapi]==2.58.0`) and frontend (`@sentry/react ^8.0.0`). Filed: 2026-04-23 QA. | `project-config.md` (§5, line ~28) | S |
