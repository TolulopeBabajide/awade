...
| M-132 | Code Quality / File Length | **`GuideViewPage.tsx` at 407 lines** — extract `Section`/`InfoCard` sub-components to `GuideViewPage.components.tsx`. Filed: 2026-05-09 code-review-agent. | `apps/frontend/src/pages/GuideViewPage.tsx` (~lines 358–407) | S | Stage: define |
| M-133 | Code Quality / Complexity | **`handleGenerateLessonResource` in `LessonPlanDetailPage.tsx` exceeds complexity threshold — ~18 decision points and 110 lines** (thresholds: 10 points, 60 lines). Root cause: AWD-M-89 unmount-guard additions were correct and necessary but left the function overloaded. **Fix**: (1) Extract polling loop (lines 131–163) into `pollUntilComplete(resourceId, isMountedRef)` helper or `useResourcePoller()` hook; (2) Extract post-success navigation + feedback (lines 165–181) into `handleGenerationSuccess()`. Would reduce orchestrator to ~40 lines and significantly simplify AWD-M-90 test coverage. Filed: 2026-05-09 code-review-agent. | `apps/frontend/src/pages/LessonPlanDetailPage.tsx:85–195` | M | Stage: define |
