...
> Last updated: 2026-05-08 (dev-agent — AWD-M-126 attempted: selected top ready item (M-126 zombie test_services.py rm). Fix requires `rm apps/backend/tests/test_services.py` — virtiofs FUSE mount in sandbox returns EPERM for all file deletions on untracked files; rm/unlink/shred all fail. Tolu: please run `rm apps/backend/tests/test_services.py` locally to complete this fix. M-126 left at stage=ready. H-65 and M-77 still blocked by Tolu's venv fix. AWD-C-13 staged-index cleared at run start (leftover M-110 docs changes). No code committed this cycle.)
...
| M-126 | Testing / Code Quality | **`test_services.py` zombie file — untracked duplicate causes 36 extra tests in local `pytest` runs.** The AWD-M-110 split commit (8c45330) used git rename detection (`test_services.py → test_auth_service.py`), which removed the file from the git index but did NOT delete it from disk. `git status` confirms it is untracked. Pytest discovers it via filesystem glob (`test_*.py`) and runs all 36 methods a second time — doubling local run time for the auth/user/context/lesson-plan suites and inflating local coverage. CI is unaffected (clean clone). **Fix**: `rm apps/backend/tests/test_services.py` — no git operation needed. Filed: 2026-05-08 code-review-agent. | `apps/backend/tests/test_services.py` | M | Stage: ready |
| L-22 | Code Quality / Style | **Inline imports and import-style inconsistency in AWD-M-110 split files.** (a) `test_auth_service.py` contains `from schemas.xxx import ...` and a redundant `from unittest.mock import patch` inside test method bodies instead of at module level — obscures the file's dependency graph and delays `ImportError` detection to execution time rather than collection. (b) Three new split files (`test_auth_service.py`, `test_user_service.py`, `test_context_service.py`) use `from services.xxx` (relative to `apps/backend/`) while `TestLessonPlanServiceSmoke` in `test_lesson_plan_service.py` uses `from apps.backend.services.xxx` — both work via conftest sys.path, but the split convention will confuse contributors. **Fix**: Move all inline imports to module level in `test_auth_service.py`; remove the duplicate `from unittest.mock import patch`; document or standardise import style in conftest. Filed: 2026-05-08 code-review-agent. | `apps/backend/tests/test_auth_service.py`, `apps/backend/tests/test_lesson_plan_service.py` | L | Stage: define |
